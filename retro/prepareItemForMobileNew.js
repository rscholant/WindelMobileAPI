...
async function prepareCondicaopgto(newDados, _dadosEmpresa) {
...
async function prepareFormapgto(newDados, _dadosEmpresa) {
  newDados = lowerCaseAllKey(newDados);
...
async function prepareDefault(objeto, _dadosEmpresas) {
  objeto = lowerCaseAllKey(objeto);
  return objeto;
}
...
async function preparePessoa(newDados, dadosEmpresa) {
  newDados = lowerCaseAllKey(newDados);
...
async function prepareProduto(newDados, _dadosEmpresa) {
  newDados = lowerCaseAllKey(newDados);
...
async function preparePedido(newDados, dadosEmpresa) {
  newDados = lowerCaseAllKey(newDados);
  ...
  if (pessoa !== null && pessoa.length > 0) {
    ...
    pessoa = await preparePessoa(pessoa, dadosEmpresa);
  }
  ...
  if (condipgto !== null && condipgto.length > 0) {
    ...
    condipgto = await prepareCondicaopgto(condipgto, dadosEmpresa);
  }
  ...
  if (formapgto !== null && formapgto.length > 0) {
    ...
    formapgto = await prepareFormapgto(formapgto, dadosEmpresa);
  }
  ...
  if (vendedor !== null && vendedor.length > 0) {
    ...
    vendedor = await preparePessoa(vendedor, dadosEmpresa);
  }
  ...
  const itensPedido = await replicacao.findAll({
    where: {
      empresa_id: dadosEmpresa.id,
  ...
  for (let i = 0; i < itensPedido.length; i += 1) {
    const produto = lowerCaseAllKey(itensPedido[i].dados);
    const product = await replicacao.findOne({
      where: {
        empresa_id: dadosEmpresa.id,
        tabela: 'PRODUTOS',
        dados: {
          IDEMPRESA: parseInt(buscaDadosEmpresa.padraoProdutos, 10),
          IDPRODUTO: parseInt(produto.idproduto, 10),
        },
      },
    });
    ...
    if (product) {
      produtosPedido.push({
        ...produto,
        ...(await prepareProduto(product.dados, dadosEmpresa)),
      });
    } else {
